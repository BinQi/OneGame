...
//			AphidLog.i("View changed: front (%d, %s), back (%d, %s), angle %s, activeIndex %d", frontIndex, frontView, backIndex, backView, angle, activeIndex);
		...
		switch (event.getAction()) {
			...
			case MotionEvent.ACTION_MOVE:
				...
				if (state == STATE_TOUCH) {
					...
					angle += angleDelta;
					if (backCards.getIndex() == -1) {
						Log.d("flipView", "滚到最后一页1");
						//下拉加载
						listener.pullDownLoad();
						if (angle >= MAX_TIP_ANGLE)
							angle = MAX_TIP_ANGLE;
	...
	//为下拉加载而用
	private OneGameListener listener = null;
	
	public OneGameListener getListener() {
		return listener;
	}

	public void setListener(OneGameListener listener) {
